...
def deployHost = "172.31.1.233" // 배포 인스턴스의 private 주소
...
pipeline {
    ...
    environment {
        ...
        ECR_URL = "043309330471.dkr.ecr.ap-northeast-2.amazonaws.com"
        ...
        stage('Deploy Changed Services to AWS EC2') {
             steps {
                   sshagent(credentials: ["deploy-key"]) {
                        ...
                        # 배포 서버로 직접 접속 시도 (compose 돌리러 감)
                        ssh -o StrictHostKeyChecking=no ubuntu@${deployHost} '
                        cd /home/ubuntu && \

                        # 시간이 지나 로그인 만료 시 필요한 명령
                        aws ecr get-login-password --region ${REGION} | docker login --user

                        # dockerk compose를 이용해서 변경된 서비스만 이미지를 pull -> 일괄 실행
